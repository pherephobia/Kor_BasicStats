...
pacman::p_load(rmdformats, ggplot2, tidyverse)
```
## 무작위 추출(Random Draws)
...
```{r}
Test10
hist(Test10, breaks = 10)
```

```{r}
## ggplot으로도 동일하게 만들 수 있습니다.
ggplot(data.frame(Test10), aes(x = Test10)) + 
  geom_histogram(aes(y=..count../sum(..count..)), 
                 binwidth = 0.02, fill = "white", color = "black",
                 position = "identity") + 
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "Histogram of Test10", x = "Test10", y = "") + 
  theme_bw()
```
...
```{r}
## ggplot으로도 동일하게 만들 수 있습니다.
ggplot(data.frame(Test1M), aes(x = Test1M)) + 
  geom_histogram(aes(y=..count../sum(..count..)), 
                 binwidth = 0.02, fill = "white", color = "black",
                 position = "identity") + 
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "Histogram of Test1M", x = "Test1M", y = "") + 
  theme_bw()
```
